...
For this tutorial we will look into finding lint in the package manifest from the Fortran package manager (`fpm`_).
...
To configure our linter we will use the `extra section <https://fpm.fortran-lang.org/en/spec/manifest.html#additional-free-data-field>`__ in the manifest which is especially reserved for tools integrating with fpm and boldly claim *extra.fpm.lint* as our configuration section.
Using the package manifest provides us with two advantages, first this document will be present in all projects using fpm and second if we can read our configuration from the manifest, we are already sure it is valid TOML.
...
Now we will setup our main program to run the linter.

.. literalinclude:: fpm-lint/app/main0.f90
   :language: fortran
   :caption: app/main.f90

We create an utility module for the *get_argument* function used to retrieve the manifest name, in most cases we can default to *fpm.toml*, but for testing it is convenient to pass an argument.
...
An example for a package name we would disallow would be *fpmLinter* as seen in the manifest below.
...
For convenience we will reexport the the other modules from the *fpm_lint* module, this allows one clean import in the main program.
Than we define the *lint_data* procedure, where we first check whether the *name* key is actually present, if not we create a message at *info* level and leave our block scope, as the all further checks rely on the presence of the entry.

We can now check whether the entry is provided as string or maybe as something else, like a literal string, which we can flag.
Furthermore, we verify that the package name uses only lowercase letter, numbers and dashes with the *verify* intrinsic.
...
To determine whether a string is actually used in the context for a key we need a way to identify all keys.
We could check all entries in the data structures by implementing a visitor which walks through all tables and checks the keys.
However, this is somewhat inefficient and we can also miss keys which are not recorded.
...
Rather than using the context to only report errors we will use it do identify keys.
...
3. create report for any key that is a string or a literal

Our implementation reflects this by first collecting an array of *toml_key* objects in *list* and than iterating over all entries checking whether they are the correct *token_kind*.
...
In a similar fashion we can identify the endings of the scopes.

We than can check whether the current scope on the top of the stack allows key paths and record those in our list.
...
In our utilities module we implement the *resize* procedure for an array of integers

.. literalinclude:: fpm-lint/src/utils.f90
   :language: fortran
   :caption: src/utils.f90
   :emphasize-lines: 6, 9-12, 16-47
   :lines: 1-50, 67-69
...
Our linter can correctly identify the *tag* entry as string in key path context and produces the appropriate message.
...
.. admonition:: Exercise
   ...
   Previously, we flagged the usage of a literal string as value for the package name, however a package manifest can contain much more string values.
   ...
   Can your make a meaningful suggestion if a literal string contains characters which must be escaped in a double quoted string?
