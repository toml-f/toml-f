...
Many programs already come with their own input formats, switching to a different format requires to establish some way to get backwards compatibility for older inputs.
When transitioning to TOML Fortran reading of the input will use the provided TOML data structures.
If the previous input format is sufficiently compatible, it can be parsed into a matching TOML data structure and allow to seamlessly use the TOML format going forward and still provide compatibility for previously written inputs.

This tutorial is meant to teach how lexing in TOML Fortran works and enable the reader to implement their own lexer for their custom format.
There is no guarantee that a custom input format can be ported by creating a custom lexer, since the format needs to fullfill some basic requirements, like providing typed values.
For the this tutorial we will choose `JSON <https://json.org/>`_ as our input format and walk through all the steps to create a new lexer from scratch.

.. note::

   The choice of JSON for this tutorial is not a coincidence.
   TOML Fortran does actually implement this lexer to parse JSON files into TOML data structures to support the encoding tests in validation suite of `BurntSushi/toml-test <https://github.com/BurntSushi/toml-test/tree/v1.1.0#implementing-an-encoder>`_.

.. important::

   This tutorial makes partially use of the internal API of TOML Fortran.
...
Every feature available in TOML to representable in the previous input format will be an incentive to switch to the new format.

.. note::

   For the actual application of the JSON parser in the validation suite this problem is solved by not using only strings to represent values and add type annotations.
   In TOML Fortran this annotations are mapped back by pruning the read JSON data structure.
...
We make sure to error out if we get a direct access or stream access unit, since we cannot reliably read those.
...
Now we can inspect the current character from the source string and decide as which token it should be labelled.
...
To wrap up the lexing we will try to identify unknown tokens as good as possible trying to advance to the next terminating character.
...
One special case to consider are literals, like strings numbers or booleans.
To not clutter the logic here we create separate routines for parsing the respecive literal values.
...
.. tip::
   ...
   Moving most of the validation logic in the tokenization simplifies the actual extraction of the value as we have to deal with less edge cases.
...
.. admonition:: Exercise
   ...
   If we want to support *null* values, how would we have to modify our lexer to produce for example and empty table ``{}`` instead, *i.e.* a *lbrace* and *rbrace* token?
...
For this purpose we will create a number of unit tests using the `test-drive <https://github.com/fortran-lang/test-drive>`_ framework.

As entry point for our tester we will use the standard wrapper for launching test suites.
...
We than can implement our checks by providing a string and a list of tokens to see whether our lexer can identify the expected tokens correctly.
...
This are only a couple of tests, we have much more cases to consider for a really robust lexer.
...
The former is wrapper which is using the *new_lexer_from_file* constructor.
...
These wrappers so far are very straight-forward, first setting up a lexer instance and invoking the *parse* procedure which will construct the actual parser instance and process the token stream.
...
Where we effectively retrieve the first child from the root table and create a deep copy of it which is than returned.

.. tip::

   An alternative approach would be to *pop* the value from the root table and return the polymorphic *toml_value* instance.
   This would have the advantage that we can actually support arrays and values at the root level with our JSON loader.
...
Now we have a working lexer which can tokenize JSON documents into TOML parsable tokens.
